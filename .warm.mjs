...
const { ensureEfficiencyTable, warmMissingEfficiency, pendingEfficiencyCount } =
  await import('./lib/aerobic-efficiency.ts').catch(() => ({}));
try{
  await c.query(`CREATE TABLE IF NOT EXISTS activity_efficiency (
    activity_id BIGINT PRIMARY KEY, pw1 DOUBLE PRECISION, hr1 DOUBLE PRECISION,
    pw2 DOUBLE PRECISION, hr2 DOUBLE PRECISION, samples INTEGER,
    computed_at TIMESTAMPTZ DEFAULT NOW())`);
  const pend = async () => Number((await c.query(`SELECT count(*)::text n FROM activity_streams s
  ...
  console.log('pending at start:', await pend());
  const SQL = `
    WITH todo AS MATERIALIZED (
      ...
      SELECT t.activity_id, dims.n samples,
  ...
  for (let i=0;i<200;i++) {
    const t0=Date.now();
    const n = Number((await c.query(SQL,[100])).rows[0].n);
    total += n;
    if (n === 0) break;
    console.log(`  batch ${i+1}: ${n} rides in ${((Date.now()-t0)/1000).toFixed(1)}s (total ${total})`);
  }
  console.log('pending at end:', await pend(), '| rows:', (await c.query(`SELECT count(*)::text n FROM activity_efficiency`)).rows[0].n);
