...
function SortIcon({ col, sortKey, sortDir }: { col: SortKey; sortKey: SortKey; sortDir: SortDir }) {
  if (col !== sortKey) return <span className="ml-1 text-ink-5">↕</span>;
  return <span className="ml-1 text-accent-hi">{sortDir === 'asc' ? '↑' : '↓'}</span>;
}

/** Placing badge — always reflects time-based rank in the full effort set. */
function PlaceBadge({ rank }: { rank: number }) {
  if (rank === 1) return (
    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-yellow-500/20 text-yellow-400 text-micro font-bold border border-yellow-500/40">1</span>
  );
  if (rank === 2) return (
    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gray-400/15 text-ink-2 text-micro font-bold border border-gray-500/30">2</span>
  );
  if (rank === 3) return (
    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-orange-700/20 text-accent-hi text-micro font-bold border border-orange-700/30">3</span>
  );
  if (rank <= 10) return (
    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-green-500/10 text-green-400 text-micro font-bold border border-green-500/20">{rank}</span>
  );
  return (
    <span className="inline-flex items-center justify-center w-6 h-6 text-ink-5 text-micro tabular-nums">{rank}</span>
  );
}
...
export default function SegmentDetail({ id }: { id: string }) {
  ...
  if (segLoad) {
    return (
      <div className="max-w-2xl md:max-w-5xl xl:max-w-7xl mx-auto px-4 py-8 space-y-4">
        {[1, 2, 3].map(i => <div key={i} className="bg-raised rounded-xl h-24 animate-pulse" />)}
      </div>
    );
  }
  ...
  if (error || !segment) {
    return (
      <div className="max-w-2xl md:max-w-5xl xl:max-w-7xl mx-auto px-4 py-8 text-center text-ink-3">
        Segment not found.{' '}
        <Link href="/activities" className="text-accent-hi hover:underline">Back</Link>
      </div>
    );
  }
  ...
  const thClass = "px-3 py-3 text-ink-4 font-medium cursor-pointer select-none hover:text-ink-2 transition-colors whitespace-nowrap";
    ...
    <div className="h-full overflow-y-auto scroll-touch">
      <div className="max-w-2xl md:max-w-5xl xl:max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-ink mb-1">{segment.name}</h1>
          ...
          <a
            ...
            className="flex-shrink-0 text-xs text-accent-hi hover:text-accent-hi border border-accent/30 rounded-lg px-3 py-1.5 transition-colors"
          >
            Strava ↗
          </a>
        </div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-4">
          <div>
            <p className="text-micro text-ink-4 uppercase tracking-wider mb-1">Distance</p>
            <p className="text-lg font-bold text-ink">{(segment.distance / 1000).toFixed(2)}<span className="text-xs text-ink-3 ml-1">km</span></p>
          </div>
          {segment.total_elevation_gain != null && (
            <div>
              <p className="text-micro text-ink-4 uppercase tracking-wider mb-1">Elev Gain</p>
              <p className="text-lg font-bold text-ink">{Math.round(segment.total_elevation_gain)}<span className="text-xs text-ink-3 ml-1">m</span></p>
            </div>
          )}
          {segment.avg_grade != null && (
            <div>
              <p className="text-micro text-ink-4 uppercase tracking-wider mb-1">Avg Grade</p>
              <p className="text-lg font-bold text-ink">{segment.avg_grade.toFixed(1)}<span className="text-xs text-ink-3 ml-1">%</span></p>
            </div>
          )}
          {segment.elevation_low != null && (
            <div>
              <p className="text-micro text-ink-4 uppercase tracking-wider mb-1">Lowest</p>
              <p className="text-lg font-bold text-ink">{Math.round(segment.elevation_low)}<span className="text-xs text-ink-3 ml-1">m</span></p>
            </div>
          )}
          {segment.elevation_high != null && (
            <div>
              <p className="text-micro text-ink-4 uppercase tracking-wider mb-1">Highest</p>
              <p className="text-lg font-bold text-ink">{Math.round(segment.elevation_high)}<span className="text-xs text-ink-3 ml-1">m</span></p>
            </div>
          )}
          {elevDiff != null && (
            <div>
              <p className="text-micro text-ink-4 uppercase tracking-wider mb-1">Elev Diff</p>
              <p className="text-lg font-bold text-ink">{elevDiff}<span className="text-xs text-ink-3 ml-1">m</span></p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-ink-3">
          {segment.climb_category != null && segment.climb_category > 0 && (
            <span>Cat <span className="text-ink font-semibold">{CLIMB_CATEGORY[segment.climb_category] ?? segment.climb_category}</span></span>
          )}
        ...
        {elevData.length > 2 && (
          <div>
            <p className="text-xs font-semibold text-ink-4 uppercase tracking-wider mb-3">Elevation Profile</p>
                ...
                <AreaChart data={elevData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id="elevGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%"  stopColor={CHART.power} stopOpacity={0.3} />
                      <stop offset="95%" stopColor={CHART.power} stopOpacity={0.05} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART.axis} vertical={false} />
                  <XAxis dataKey="dist" tickFormatter={v => `${v}km`} tick={{ fill: CHART.axisText, fontSize: 10 }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
                  <YAxis tickFormatter={v => `${v}m`} tick={{ fill: CHART.axisText, fontSize: 10 }} axisLine={false} tickLine={false} width={44} />
                  <Tooltip
                  ...
                  />
                  <Area type="monotone" dataKey="alt" stroke={CHART.power} strokeWidth={2} fill="url(#elevGrad)" dot={false} activeDot={{ r: 3, fill: CHART.power }} />
                </AreaChart>
              </ResponsiveContainer>
                )}
              </EnlargeableChart>
        ...
        <div>
          ...
          <div className="flex items-center justify-between mb-3 gap-3">
            ...
            {!effLoad && efforts.length > 0 && (
              <button
                ...
                className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-lg border transition-colors ${
                  showFilters || filterCount > 0
                    ? 'bg-accent/15 text-accent-hi border-accent/40'
              ...
              >
                ...
                Filter
                {filterCount > 0 && (
                  <span className="bg-accent text-ink text-micro font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">
                    {filterCount}
                  </span>
                )}
              </button>
          ...
          {/* Filter panel */}
          {showFilters && (
            <div className="bg-surface border border-line rounded-xl p-4 mb-3 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                {/* Date range */}
                <div className="col-span-2">
                  <p className="text-micro text-ink-4 uppercase tracking-wider mb-1.5">Date range</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-micro text-ink-5 block mb-1">From</label>
                      <input
                        type="date"
                        value={filters.dateFrom}
                        onChange={e => setFilter('dateFrom', e.target.value)}
                        className="w-full bg-raised border border-line-strong rounded px-2 py-1.5 text-xs text-ink focus:outline-none focus:border-accent"
                      />
                    </div>
                    <div>
                      <label className="text-micro text-ink-5 block mb-1">To</label>
                      <input
                        type="date"
                ...
                {/* Time range */}
                <div>
                  <p className="text-micro text-ink-4 uppercase tracking-wider mb-1.5">Time range</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-micro text-ink-5 block mb-1">Min (m:ss)</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="text-micro text-ink-5 block mb-1">Max (m:ss)</label>
                      <input
                        type="text"
                ...
                {/* Power range */}
                <div>
                  <p className="text-micro text-ink-4 uppercase tracking-wider mb-1.5">Power range (W)</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-micro text-ink-5 block mb-1">Min</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="text-micro text-ink-5 block mb-1">Max</label>
                      <input
                        type="number"
          ...
          {effLoad ? (
            <div className="space-y-2">
              {[1,2,3,4,5].map(i => <div key={i} className="h-12 bg-raised rounded-xl animate-pulse" />)}
            </div>
          ) : efforts.length === 0 ? (
            <div className="bg-raised/40 rounded-xl p-6 text-center space-y-3">
              <p className="text-ink-3 text-sm">No efforts found yet.</p>
              {remaining > 0 && (
                <p className="text-ink-4 text-xs">{remaining} activities still need to be scanned.</p>
              )}
              <button
                onClick={() => {
                  setEffLoad(true);
                ...
                className="text-xs text-accent-hi hover:text-accent-hi border border-accent/30 rounded px-3 py-1.5 transition-colors"
              >
                Scan activity history
              </button>
              {debug && (
                <pre className="text-left text-xs text-ink-3 bg-surface rounded p-3 overflow-auto max-h-48 mt-2">
            ...
            <div className="bg-surface rounded-xl border border-line overflow-x-auto scroll-touch">
              <table className="text-sm w-full whitespace-nowrap">
                <thead>
                  <tr className="border-b border-line">
                    {/* Placing */}
                    <th className="px-3 py-3 text-ink-4 font-medium text-center w-8">#</th>
                  ...
                  {filtered.map((e, rowIdx) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-line/60 last:border-0 cursor-pointer transition-colors hover:bg-raised/60 ${rowBg(rank, rowIdx % 2 !== 0)}`}
                      >
                        {/* Placing */}
                        ...
                        {/* Date */}
                        <td className="px-3 py-3 text-ink-2 text-left">
                          {date.toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })}
                        </td>

                        {/* Time */}
                        <td className="px-3 py-3 text-right tabular-nums">
                          <span className={`font-bold ${rank === 1 ? 'text-yellow-300' : rank <= 3 ? 'text-accent-hi' : 'text-ink'}`}>
                            {fmt(e.elapsed_time)}
                          </span>
                          {isPR && (
                            <span className="ml-1.5 text-micro bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 px-1 py-0.5 rounded font-bold">PR</span>
                          )}
                          {isTop && !isPR && (
                            <span className="ml-1.5 text-micro bg-accent/20 text-accent-hi border border-accent/30 px-1 py-0.5 rounded font-bold">Top {e.kom_rank}</span>
                          )}
                        </td>
                        ...
                        <td className="px-3 py-3 text-right tabular-nums text-ink-2">
                          {e.average_watts
                            ? <>{Math.round(e.average_watts)}<span className="text-xs text-ink-4 ml-0.5">W</span></>
                        ...
                        <td className="px-3 py-3 text-right tabular-nums text-ink-2">
                          {e.average_heartrate
                            ? <>{Math.round(e.average_heartrate)}<span className="text-xs text-ink-4 ml-0.5">bpm</span></>
                            : <span className="text-ink-5">—</span>}
                        </td>
                        ...
                        <td className="px-3 py-3 text-right tabular-nums text-ink-2">
                          {e.max_heartrate
                            ? <>{Math.round(e.max_heartrate)}<span className="text-xs text-ink-4 ml-0.5">bpm</span></>
                            : <span className="text-ink-5">—</span>}
                        </td>
                        ...
                        <td className="px-3 py-3 text-right">
                          {e.wind_speed != null ? (
                            <span className="text-ink-2 tabular-nums">
                              {e.wind_speed}<span className="text-xs text-ink-4 ml-0.5">km/h</span>
                              {e.wind_compass && (
                                <span className="ml-1.5 text-ink-3 text-xs">
              ...
              {filtered.length === 0 && filterCount > 0 && (
                <div className="py-8 text-center text-ink-4 text-sm">
                  No efforts match your filters.{' '}
                  <button onClick={() => setFilters(EMPTY_FILTERS)} className="text-accent-hi hover:text-accent-hi transition-colors">Clear</button>
                </div>
              )}
            </div>
          )}
