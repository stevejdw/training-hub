...
const AXIS_TICK = { fill: '#6b7280', fontSize: 10 } as const;
const TOOLTIP_STYLE = { background: '#111827', border: '1px solid #374151', borderRadius: 8, fontSize: 12 } as const;
  ...
  if (loading) {
    return (
      <div className="space-y-3">
        {[1, 2, 3].map(i => <div key={i} className="h-36 animate-pulse bg-gray-800 rounded-xl" />)}
      </div>
    );
  }
  ...
  if (failed || rows.length === 0 || (!hasWatts && !hasHr && !hasAlt)) {
    return (
      <div className="bg-gray-800/50 border border-gray-700 border-dashed rounded-xl p-8 text-center">
        <p className="text-gray-500 text-sm">No stream data for this activity</p>
      </div>
    );
  }
  ...
  function chartPanel(title: string, unit: string, children: React.ReactNode) {
    return (
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-3">
        <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">{title}{unit ? ` (${unit})` : ''}</p>
        {children}
      </div>
    );
  }
        ...
        <div className="flex bg-gray-800 rounded-lg p-0.5 gap-0.5">
          {(['time', 'distance'] as XMode[]).map(m => (
            <button
              ...
              className={`px-2 py-1 rounded-md text-[10px] font-medium transition-colors disabled:opacity-30 ${
                xMode === m ? 'bg-orange-500 text-white' : 'text-gray-400 hover:text-white'
              }`}
            >
      ...
      {hasWatts && chartPanel('Power', 'W', (
        <ResponsiveContainer width="100%" height={140}>
          <ComposedChart {...sharedChartProps}>
            ...
            <Line type="monotone" dataKey="watts" stroke="#f97316" strokeWidth={1.5} dot={false} isAnimationActive={false} connectNulls />
          </ComposedChart>
        </ResponsiveContainer>
      ))}
      ...
      {hasHr && chartPanel('Heart rate', 'bpm', (
        <ResponsiveContainer width="100%" height={120}>
          <ComposedChart {...sharedChartProps}>
            ...
            <Line type="monotone" dataKey="hr" stroke="#f87171" strokeWidth={1.5} dot={false} isAnimationActive={false} connectNulls />
          </ComposedChart>
        </ResponsiveContainer>
      ))}
      ...
      {hasAlt && chartPanel('Elevation', 'm', (
        <ResponsiveContainer width="100%" height={110}>
          <ComposedChart {...sharedChartProps}>
            ...
            <Area type="monotone" dataKey="alt" stroke="#60a5fa" strokeWidth={1.5} fill="#60a5fa" fillOpacity={0.15} isAnimationActive={false} connectNulls />
          </ComposedChart>
        </ResponsiveContainer>
      ))}

      {/* Overview + zoom brush over the full ride */}
      <div className="bg-gray-900 rounded-xl border border-gray-800 px-3 pt-2 pb-1">
        <div className="flex items-center justify-between mb-1">
          <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Zoom</p>
          {window_ && (
            <button onClick={() => setWindow(null)} className="text-[10px] text-orange-400 hover:text-orange-300 transition-colors">
              Reset
            </button>
          )}
        </div>
          ...
          <ComposedChart data={rows} margin={{ top: 0, right: 8, left: 44, bottom: 0 }}>
            ...
            >
              <ComposedChart data={rows}>
                <Area type="monotone" dataKey={hasAlt ? 'alt' : hasWatts ? 'watts' : 'hr'} stroke="#4b5563" fill="#374151" isAnimationActive={false} connectNulls />
              </ComposedChart>
            </Brush>
          </ComposedChart>
        </ResponsiveContainer>
