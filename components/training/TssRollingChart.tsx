...
function CustomTooltip({ active, payload, label }: any) {
  ...
  if (p.date) {
    return (
      <div className="bg-surface border border-line-strong rounded-lg px-3 py-2 text-xs shadow-lg space-y-0.5">
        <p className="text-ink-3">{label} {p.date}</p>
        <p className="text-accent-hi">{p.actual_tss} TSS <span className="text-ink-4 font-normal">actual</span></p>
        {p.target_tss > 0 && (
          <p className="text-blue-400">{p.target_tss} TSS <span className="text-ink-4 font-normal">target</span></p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="bg-surface border border-line-strong rounded-lg px-3 py-2 text-xs shadow-lg space-y-0.5">
      <p className="text-ink-3">Week of {label}</p>
      <p className="text-accent-hi">{p.actual_tss} TSS <span className="text-ink-4 font-normal">actual</span></p>
      {p.target_tss > 0 && (
        <p className="text-blue-400">
          {p.target_tss} TSS <span className="text-ink-4 font-normal">target</span>
          {p.is_recovery && <span className="ml-1 text-green-400">· recovery</span>}
        </p>
      )}
      {p.target_source === 'plan' && <p className="text-micro text-ink-5">from training plan</p>}
      {p.target_source === 'formula' && <p className="text-micro text-ink-5">from formula</p>}
      {p.high_duration_recovery_warning && (
        <p className="text-amber-400 font-medium pt-1 border-t border-line-strong mt-1">
...
export default function TssRollingChart({ compact }: { compact?: boolean }) {
  ...
  if (compact) {
    return (
      <div className="bg-surface rounded-xl border border-line p-3 space-y-2">
        {/* Weekly summary — current week only */}
        <div className="grid grid-cols-3 gap-1.5">
          <div className="bg-raised/60 rounded-lg p-1.5 text-center">
            <p className="text-micro text-ink-4 uppercase tracking-wider">Actual</p>
            <p className="text-sm font-bold text-accent-hi tabular-nums">{totalActual}</p>
          </div>
          <div className="bg-raised/60 rounded-lg p-1.5 text-center">
          ...
          <div className="bg-raised/60 rounded-lg p-1.5 text-center">
            <p className="text-micro text-ink-4 uppercase tracking-wider">On track</p>
            <p className={`text-sm font-bold tabular-nums ${
              onTrack == null ? 'text-ink-4' :
        ...
        {loading && points.length === 0 ? (
          <div className="h-24 animate-pulse bg-raised rounded-lg" />
        ) : (
          <ResponsiveContainer width="100%" height={100}>
            <ComposedChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }} barCategoryGap="20%">
              <CartesianGrid strokeDasharray="3 3" stroke={CHART.grid} vertical={false} />
              <XAxis dataKey="label" tick={{ fill: CHART.axisText, fontSize: 8 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: CHART.axisText, fontSize: 8 }} axisLine={false} tickLine={false} width={24} />
              <Tooltip content={<CustomTooltip />} cursor={{ fill: CHART.axis, fillOpacity: 0.2 }} />
              <Bar dataKey="actual_tss" fill={CHART.power} name="actual_tss" radius={[2,2,0,0]} />
              <Line dataKey="target_tss" stroke={CHART.hr} name="target_tss" strokeWidth={1.5}
                    dot={{ r: 2, fill: CHART.hr, strokeWidth: 0 }}
  ...
  return (
    <div className="bg-surface rounded-xl border border-line p-4 space-y-3">
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold text-ink-4 uppercase tracking-wider">Rolling TSS</p>
          <p className="text-micro text-ink-5 mt-0.5">{modeLabel}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="text-micro text-accent-hi hover:text-accent-hi border border-accent/30 hover:border-accent/60 rounded px-2 py-1 transition-colors"
          >
            Edit
          </button>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-raised/60 rounded-lg p-2 text-center">
          <p className="text-micro text-ink-4 uppercase tracking-wider">Actual</p>
          <p className="text-lg font-bold text-accent-hi tabular-nums">{totalActual}</p>
        </div>
        <div className="bg-raised/60 rounded-lg p-2 text-center">
          <p className="text-micro text-ink-4 uppercase tracking-wider">Target</p>
      ...
      {/* Chart */}
      {loading && points.length === 0 ? (
        <div className="h-44 animate-pulse bg-raised rounded-lg" />
      ) : (
        <EnlargeableChart title="Rolling TSS" subtitle={modeLabel} controls={
          ...
          <ComposedChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }} barCategoryGap="20%">
            ...
            <YAxis tick={{ fill: CHART.axisText, fontSize: 10 }} axisLine={false} tickLine={false} width={32} />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: CHART.axis, fillOpacity: 0.2 }} />
            <Legend
              iconSize={8}
              formatter={value => (
            ...
            />
            <Bar  dataKey="actual_tss" fill={CHART.power} name="actual_tss" radius={[3,3,0,0]} />
            ...
            {isDayView && config?.mode === 'formula' && totalTarget > 0 ? (
              <ReferenceLine y={totalTarget} stroke={CHART.hr} strokeWidth={2}
                strokeDasharray="6 3"
                label={{ value: `Weekly target: ${totalTarget}`, position: 'right', fill: CHART.hr, fontSize: 10 }} />
            ) : (
              <Line dataKey="target_tss" stroke={CHART.hr} name="target_tss" strokeWidth={2}
                    dot={{ r: 4, fill: CHART.hr, strokeWidth: 0 }}
      ...
      <div className="flex items-center justify-center gap-4 select-none">
        ...
        <span className="text-xs text-ink-2 font-medium tabular-nums text-center min-w-[10rem]">{rangeLabel}</span>
        <button
          onClick={() => setOffset(o => Math.max(0, o - weeks))}
          disabled={offset <= 0}
          className={`p-1.5 rounded transition-colors ${
        ...
        <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${chartData.length}, 1fr)` }}>
          {chartData.map((p, i) => (
            <div key={p.week_start ?? `w${i}`} className="text-center">
              {p.is_recovery && <span className="text-micro text-green-400 font-medium uppercase">recovery</span>}
            </div>
          ))}
        </div>
      )}
      ...
      {!isDayView && chartData.some(p => p.high_duration_recovery_warning) && (
        <div
          role="status"
          className="rounded-lg border border-amber-500/35 bg-amber-500/10 px-3 py-2 text-mini text-amber-200/95 leading-snug"
        >
          High Duration Warning: May Delay Recovery.
        </div>
      )}
...
function TssGoalEditModal({ initial, onClose, onSaved }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4"
         onClick={onClose}>
      <div className="bg-surface border border-line-strong rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto"
        ...
        <div className="p-5 space-y-4">
          {/* Mode */}
          <div>
            <label className="block text-mini font-semibold text-ink-3 uppercase tracking-wider mb-2">Source</label>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setField('mode', 'plan')}
                className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors ${
                  cfg.mode === 'plan'
                    ? 'bg-accent/20 text-accent-hi border border-accent/50'
                    : 'bg-raised text-ink-4 border border-transparent hover:text-ink-2'
                }`}>
                From training plan
              </button>
            ...
            <p className="text-micro text-ink-5 mt-1.5">
              {cfg.mode === 'plan'
          ...
          {cfg.mode === 'formula' && (
            <>
              {/* Goal */}
              <div>
                <label className="block text-mini font-semibold text-ink-3 uppercase tracking-wider mb-1">
                ...
                <div className="flex items-center gap-2">
                  <input type="number" min={50} max={2000} step={10}
                    ...
                    className="flex-1 bg-raised border border-line-strong rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:border-accent tabular-nums" />
                  <span className="text-xs text-ink-4">TSS / week</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="number" min={0} max={50} step={0.5}
                    ...
                    className="flex-1 bg-raised border border-line-strong rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:border-accent tabular-nums" />
                  <span className="text-xs text-ink-4">% per build week (compound)</span>
                </div>
              </div>

              {/* Block size */}
              <div>
                <label className="block text-mini font-semibold text-ink-3 uppercase tracking-wider mb-2">
                  Block size
                </label>
                <div className="grid grid-cols-2 gap-2">
                  {[3, 4].map(n => (
                    <button key={n} onClick={() => setField('block_weeks', n as 3 | 4)}
                      ...
                      {n}-week block
                      <span className="block text-micro text-ink-5 mt-0.5">{n - 1} build + 1 recovery</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="number" min={20} max={100} step={5}
                    ...
                    className="flex-1 bg-raised border border-line-strong rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:border-accent tabular-nums" />
                  <span className="text-xs text-ink-4">% of last build week</span>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-micro text-ink-5 mt-1">
              ...
              <div>
                ...
                <div className="bg-raised/40 rounded-lg p-3 grid grid-cols-4 gap-2">
                  {preview.map((p, i) => (
                    <div key={i} className="text-center">
                      <p className="text-micro text-ink-5">{p.label}</p>
                      <p className={`text-xs font-bold tabular-nums ${p.recovery ? 'text-green-400' : 'text-blue-400'}`}>
                        {p.tss}
                      </p>
                      {p.recovery && <p className="text-micro text-green-500 uppercase">rec</p>}
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="px-5 py-3 border-t border-line flex justify-end gap-2">
          ...
          <button onClick={save} disabled={saving}
            className="px-4 py-2 rounded-lg bg-accent hover:bg-accent-hi text-ink text-sm font-medium transition-colors disabled:opacity-50">
            {saving ? 'Saving…' : 'Save'}
          </button>
        </div>
      </div>
