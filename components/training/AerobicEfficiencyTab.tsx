...
function EfTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-surface border border-line-strong rounded-lg px-3 py-2 text-xs shadow-lg space-y-0.5">
      <p className="text-ink-3">{new Date(row.x).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
      <p className="text-ink-2 truncate max-w-[200px] font-medium">{row.name}</p>
      <p className="text-accent-hi font-semibold pt-1">EF {ef}</p>
      <p className="text-micro text-ink-4">{row.avg_watts}W avg · {row.np}W NP · {row.avg_hr} bpm</p>
      {row.hrv_low === true && <p className="text-micro text-yellow-400">⚠ Low HRV day</p>}
      <p className="text-micro text-ink-5 mt-0.5">Click for ride detail</p>
    </div>
  );
}
      ...
      >
        ...
        {ride.summary_polyline && (
          <div className="border-b border-line">
            <ActivityMap polyline={ride.summary_polyline} className="w-full h-44 rounded-none overflow-hidden bg-raised" thumbnail />
          </div>
        )}

        {/* Stats row */}
        <div className="grid grid-cols-5 gap-2 p-4 border-b border-line">
          {[
        ...
        <div className="p-4 border-b border-line">
          <p className="text-mini text-ink-4 uppercase tracking-wider mb-3">Aerobic Decoupling — First vs Second Half</p>
        ...
        {/* EF time series */}
        <div className="p-4">
          <p className="text-mini text-ink-4 uppercase tracking-wider mb-3">Efficiency Factor (EF) Over Ride</p>
          {loading ? (
          ...
          {brushStats && (
            <div className="mt-3 bg-blue-500/10 border border-blue-500/30 rounded-xl p-3">
              <div className="flex items-center justify-between mb-2">
                <p className="text-micro text-blue-400 uppercase tracking-wider font-medium">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="text-center">
                  <p className="text-micro text-ink-4 uppercase">Avg Power</p>
                  <p className="text-sm font-bold text-accent-hi">{brushStats.avgWatts}W</p>
                </div>
                <div className="text-center">
                  <p className="text-micro text-ink-4 uppercase">Avg HR</p>
                  <p className="text-sm font-bold text-blue-400">{brushStats.avgHr} bpm</p>
                </div>
                <div className="text-center">
                  <p className="text-micro text-ink-4 uppercase">EF</p>
                  <p className="text-sm font-bold text-ink">{brushStats.ef}</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-4 border-t border-line flex items-center justify-between">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 text-xs font-medium text-accent-hi hover:text-accent-hi transition-colors"
          >
            View full activity
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-raised/60 rounded-xl p-3 text-center">
          <p className="text-micro text-ink-4 uppercase tracking-wider mb-1">Steady rides</p>
          <p className="text-2xl font-bold text-ink">{scatterData.length}</p>
          <p className="text-micro text-ink-4 mt-0.5">VI &lt; 1.10 · ≥60 min</p>

        </div>

        <div className="bg-raised/60 rounded-xl p-3 text-center">
          <p className="text-micro text-ink-4 uppercase tracking-wider mb-1">Avg decoupling</p>
          ...
          <p className="text-micro text-ink-4 mt-0.5">{avgDecoupling < 3 ? 'Good' : avgDecoupling < 5 ? 'OK' : 'Drift'}</p>
        </div>
        <div className="bg-raised/60 rounded-xl p-3 text-center">
          <p className="text-micro text-ink-4 uppercase tracking-wider mb-1">Avg EF</p>
          {avgEf !== null ? (
            <>
              <p className="text-2xl font-bold text-accent-hi">{avgEf.toFixed(3)}</p>
              {trendPct !== null && (
                <p className={`text-micro mt-0.5 ${trendPct >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="bg-surface rounded-xl border border-line p-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-semibold text-ink-4 uppercase tracking-wider">Aerobic Efficiency (EF)</p>
          <ChartNav
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          <div className="flex gap-1.5">
            {([
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-micro font-medium transition-colors ${
                  zoneFilter === z.key
          ...
          {trendPct !== null && (
            <p className={`text-mini font-medium ${trendPct >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {trendPct >= 0 ? '↑' : '↓'} {Math.abs(trendPct).toFixed(1)}% over {rangeLabel}
            </p>
          )}
        </div>
        {zoneFilter !== 'all' && zoneLabel && (
          <p className="text-mini text-blue-400/70 mb-2">
        ...
        <p className="text-mini text-ink-5 mb-1">
        ...
        {error ? (
          ...
          <div className="h-64 flex items-center justify-center text-ink-4 text-sm text-center px-4">
