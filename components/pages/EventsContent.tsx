...
function EventCard({ event }: { event: EventGoal }) {
  ...
  const daysColor = today ? 'text-accent-hi font-bold'
  ...
  return (
    <Link
      ...
      className={`block bg-surface border rounded-2xl overflow-hidden transition-colors hover:border-line-strong active:bg-raised/60 ${
        past ? 'border-line/60 opacity-60' : 'border-line'
      }`}
    >
      {/* Top bar: days to go */}
      {daysLabel && (
        <div className={`flex items-center justify-end px-4 pt-3 text-mini font-semibold uppercase tracking-wider ${daysColor}`}>
          {daysLabel}
        </div>
      )}
      ...
      <div className="px-4 pb-4 pt-2 space-y-2">
        {/* Name + date */}
        <div>
          <h3 className="text-lg font-bold text-ink leading-tight">{event.name || 'Unnamed event'}</h3>
          <div className="flex items-center gap-3 mt-0.5">
            {event.date && (
              <span className="text-sm text-ink-3">{fmtDate(event.date)}</span>
            )}
            {event.location && (
              <span className="flex items-center gap-1 text-sm text-ink-4">
        ...
        {event.goal && (
          <p className="text-sm text-ink-3 leading-snug">
            <span className="text-ink-5 text-xs uppercase tracking-wider mr-1.5">Goal</span>
            {event.goal}
          </p>
        )}
        ...
        {route && (
          <div className="flex items-center gap-4 pt-1">
            ...
            {event.pacing_strategy?.est_time_min ? (
              <span className="flex items-center gap-1.5 text-xs text-accent-hi font-medium">
              ...
              <span className="text-xs text-ink-5">No pacing set</span>
            )}
          </div>
        )}
        ...
        <div className="flex justify-end">
          <svg className="w-4 h-4 text-ink-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export default function EventsContent() {
  ...
  const plusBtn = (
    <button
      ...
      className="w-8 h-8 flex items-center justify-center rounded-full bg-accent hover:bg-accent-hi text-ink transition-colors"
    >
        ...
        <div className="max-w-2xl md:max-w-5xl xl:max-w-7xl mx-auto px-4 py-4 md:px-8 md:py-8 space-y-6">
          ...
          {upcoming.length > 0 ? (
            <section className="space-y-3">
              <h2 className="text-xs font-semibold uppercase tracking-wider text-ink-4 px-0.5">Upcoming</h2>
              {upcoming.map((e, i) => <EventCard key={e.id ?? i} event={e} />)}
            </section>
          ) : (
            <div className="bg-surface border border-line border-dashed rounded-2xl p-10 text-center space-y-2">
              <p className="text-ink-3 text-sm">No upcoming events.</p>
              <p className="text-ink-5 text-xs">Tap + to add your first event.</p>
            </div>
          )}
          ...
          {past.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-xs font-semibold uppercase tracking-wider text-ink-4 px-0.5">Past</h2>
